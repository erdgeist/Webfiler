...
def admin():
    ...
    return render_template('admin.html', users = users, tree = make_tree(basedir, publicdir), url_root = url_root, documentsdir = documentsdir)

@app.route('/admin/'+documentsdir+'/<user>', methods=['GET'])
def admin_dokumente(user):
    return render_template('mandant.html', admin = 'admin/', user = user, tree = make_tree(basedir, path.join(documentsdir, user)), documentsdir = documentsdir)
...
def mandant(user):
    return render_template('mandant.html', admin = '', user = user, tree = make_tree(basedir, path.join(documentsdir, user)), documentsdir = documentsdir)
...
def custom_static(user, filename):
    return send_from_directory(path.join(basedir, documentsdir), path.join(user, filename))
