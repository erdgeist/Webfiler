...
class GPGEncryption:
    def __init__(
        ...
        if sender_email_address:
            input_data = self.gpg.gen_key_input(
                name_email=sender_email_address, passphrase=passphrase
            )
            if new_dir:
        ...
        for i in range(0, retries):
            ...
            for k in found_keys:
                if k and k["keyid"] == fingerprint:
                    ...
                    return self.gpg.recv_keys(
                        self.key_server, fingerprint
    ...
    def encrypt_text(self, recipient_fprint, text):
        ...
        status = self.gpg.encrypt(
            text, recipients=[recipient_fprint], always_trust=True
        )
